...
module.exports = function (client, options) {
	...
	function play(msg, suffix) {
		...
		msg.channel.sendMessage(':mag_right: **Searching**  `' + suffix + '` . . .').then(response => {
			...
			YoutubeDL.getInfo(searchstring, ['-q', '--no-warnings', '--force-ipv4'], (err, info) => {
				...
				if (err || info.format_id === undefined || info.format_id.startsWith('0')) {
					return response.edit('```Sorry I could not find that certian video!```');
				}

				info.requester = msg.author.id;

				// Queue the video.
				response.edit(':musical_note: Added `' + info.title + '` to the queue!').then(() => {
					queue.push(info);
	...
	function queue(msg, suffix) {
		...
		msg.channel.sendMessage('```Queue``` (' + queueStatus + '):\n' + '`' + text + '`');
	}
	...
	function executeQueue(msg, queue) {
		...
		new Promise((resolve, reject) => {
			...
			// Play the video.
			msg.channel.sendMessage(':notes: **Now Playing** `' + video.title + '` :notes:').then(() => {
